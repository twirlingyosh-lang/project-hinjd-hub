...
const HydraulicSchematic = () => {
  ...
  const buildExportSvgString = useCallback(() => {
    ...
    // Add white background rect as first child for print
  ...
  const handleExportPNG = useCallback(async () => {
    ...
    try {
      ...
      toast.error('Failed to export schematic');
    }
  ...
  const handlePrint = useCallback(() => {
    const svgString = buildExportSvgString();
    if (!svgString) return;
    const componentsList = currentSchematic.components.map(c =>
    ...
    printWindow.document.write(`<!DOCTYPE html><html><head><title>${currentSchematic.name} Hydraulic Schematic</title><style>
      body{font-family:Arial,sans-serif;margin:40px;color:#333}
      h1{font-size:22px;margin-bottom:4px}
      .subtitle{color:#666;font-size:14px;margin-bottom:24px}
      .svg-wrap{background:#0f172a;border-radius:8px;padding:16px;margin-bottom:24px}
      svg{width:100%;height:auto}
      table{border-collapse:collapse;width:100%;font-size:12px;margin-top:8px}
      th{background:#f1f5f9;padding:6px 8px;border:1px solid #ddd;text-align:left;font-weight:600}
      .footer{margin-top:32px;font-size:11px;color:#999;border-top:1px solid #eee;padding-top:12px}
      @media print{body{margin:20px}.no-print{display:none}}
    </style></head><body>
      <h1>${currentSchematic.name} Hydraulic System</h1>
      <p class="subtitle">${currentSchematic.description}</p>
      <div class="svg-wrap">${svgString}</div>
      <h2 style="font-size:16px;margin-top:24px">Component Details</h2>
      <table><thead><tr><th>Component</th><th>Type</th><th>Part Number</th><th>Manufacturer</th><th>Pressure</th><th>Flow</th></tr></thead><tbody>${componentsList}</tbody></table>
      <div class="footer">Generated from HINJD Ecosystem Hub — ${new Date().toLocaleDateString()}</div>
