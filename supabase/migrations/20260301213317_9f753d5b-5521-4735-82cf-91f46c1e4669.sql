...
-- Fix all RLS policies to explicitly use TO authenticated to eliminate anonymous access warnings
...
-- Also fix the new module policy to TO authenticated
...
CREATE POLICY "Module users view diagnostic logic" ON public.diagnostic_logic FOR SELECT TO authenticated USING (public.has_module_access('equipment_diagnostics') OR public.has_role(auth.uid(), 'admin'::app_role));

-- email_leads (keep anon INSERT, fix admin policies)
